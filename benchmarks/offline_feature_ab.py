...
Method (mirrors the production training pipeline):
  ...
  - TPOT contamination prevented by column-level assertion before training
    (candidate feature must not appear in TPOT's feature list)
...
# Usage: CANDIDATE_FORMULAS = {"my_feature": lambda df: df["col_a"] * df["col_b"]}
# Then run: python benchmarks/offline_feature_ab.py --trace t.jsonl --feature my_feature
CANDIDATE_FORMULAS = {}
...
def add_derived_features(df: pd.DataFrame, feature: str | None = None) -> pd.DataFrame:
    df = df.copy()
    df.attrs["raw_columns"] = set(df.columns)
...
def _resolve_production_columns(df: pd.DataFrame, model_type: str) -> list[str] | None:
    ...
    try:
        ...
        print(
            f"WARNING: production feature engineering raised {type(e).__name__}: {e} — using fallback", file=sys.stderr
        )
        return None


def _present_features(candidates: list[str], raw_columns: set[str]) -> list[str]:
    """Features actually recorded in the raw trace (not just zero-filled by add_derived_features)."""
    return [col for col in candidates if col in raw_columns]


def _tif_enabled() -> bool:
    """Mirror production's settings.ENABLE_TOKEN_IN_FLIGHT_FEATURES (training_server.py:388-395)."""
    return os.getenv("LATENCY_ENABLE_TOKEN_IN_FLIGHT_FEATURES", "true").lower() == "true"


def _enc_enabled() -> bool:
    """Mirror production's encoder feature gate (default true, aligned with PR #59)."""
    return os.getenv("LATENCY_ENABLE_ENCODER_FEATURES", "true").lower() == "true"
...
def _resolve_ttft_features(df: pd.DataFrame, feature: str | None) -> tuple[list[str], list[str]]:
    ...
    Column order is load-bearing: XGBoost feature indices must match between
    training and prediction. Inserting a column at the wrong position silently
    changes model behavior. Order must match training_server.py:412-416.
    """
    base = _resolve_production_columns(df, "ttft")
    if base is None:
        raw_columns = df.attrs.get("raw_columns", set(df.columns))
        tif = CONDITIONAL_FEATURES if _tif_enabled() else []
        enc = _present_features(ENCODER_FEATURES, raw_columns) if _enc_enabled() else []
        base = (
...
def shap_analysis(results: dict, feature: str, df: pd.DataFrame, outdir: Path) -> dict | None:
    ...
    if total <= 0:
        print("SHAP importances are all zero — skipping SHAP plot (no signal to rank).", file=sys.stderr)
        return None
...
def main() -> None:
    ...
    raw_columns = df.attrs.get("raw_columns", set(df.columns))
    if args.feature not in df.columns or (
        args.feature in CONDITIONAL_FEATURES + ENCODER_FEATURES + ["num_tokens_generated"]
        and args.feature not in raw_columns
    ):
        sys.exit(
            f"Feature '{args.feature}' not found in raw trace. Available columns: "
            f"{sorted(c for c in raw_columns if not c.startswith('actual_'))}"
        )
    ...
    if real_ql_summary["delta_exceeds_seed_std"] and real_delta > 0:
        ...
        if args.json_output:
            print(
                json.dumps(
                    {"schema_version": SCHEMA_VERSION, "feature": args.feature, "gate": "REGRESSION", "reason": reason}
                )
            )
        sys.exit(1)
