...
Stdlib only -- runs in any Python 3.11+ image (matches this repo's requires-python), no pip installs needed.
...
if urlparse(FORWARD_URL).scheme not in ("http", "https"):
    raise ValueError(f"FORWARD_URL must use http/https, got: {FORWARD_URL!r}")
    ...
    def _proxy(self):
        ...
        if self.command == "POST" and self.path == "/add_training_data_bulk" and body:
            try:
                ...
                logging.error("Failed to record entries (forwarding continues): %s", e)
